...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # TODO add action A level to the planning graph as described in the Russell-Norvig text (done)
        ...
        total_actions = self.all_actions
        literal_list = [x.symbol for x in state_level]
        total_actions += self.noop_actions(literal_list)
        ...
        self.a_levels[level] = set(filter(lambda x: x.parents != set(), self.a_levels[level]))
    ...
    def add_literal_level(self, level):
        ...
        """
        # TODO add literal S level to the planning graph as described in the Russell-Norvig text (done)
        ...
        for a_node in self.a_levels[level - 1]:
            for eff_node in a_node.effnodes:
                self.s_levels[level].add(PgNode_s(eff_node.symbol, eff_node.is_pos))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff_1 in node_a1.effnodes:
            for eff_2 in node_a2.effnodes:
                if eff_1.symbol == eff_2.symbol and eff_1.is_pos != eff_2.is_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pre_1 in node_a1.prenodes:
            for eff_2 in node_a2.effnodes:
                if pre_1.symbol == eff_2.symbol and pre_1.is_pos != eff_2.is_pos:
                    return True
        for pre_2 in node_a2.prenodes:
            for eff_1 in node_a1.effnodes:
                if pre_2.symbol == eff_1.symbol and pre_2.is_pos != eff_1.is_pos:
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # TODO test for negation between nodes (done)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for pre_1 in node_s1.parents:
            for pre_2 in node_s2.parents:
                if not pre_1.is_mutex(pre_2):
                    return False
        return True
